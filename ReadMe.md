...
> **Status:** Phase 2 Complete (Hardened & Tested)

A production-ready, environment-agnostic AI pipeline that ingests documents, creates embeddings, and performs Retrieval-Augmented Generation (RAG) using Groq and a decoupled vector store.
...
```markdown
├── src/
│   ├── ingestion.py       # Handles loading (PDF/TXT) & cleaning logic (Hardened)
...
│   └── interfaces.py      # Abstract Base Classes (Strict Contracts)
├── tests/                 # Automated Test Suite (Phase 2)
│   └── test_pipeline.py   # Unit Tests for Ingestion & Validation
├── static/                # Frontend UI (HTML/CSS/JS)
├── main.py                # FastAPI Service & Async Hooks
├── HANDOVER.md            # Engineering Handover Notes
...
Clone the repository and install dependencies (including testing tools):
...
3. **Interact:**
* **Upload:** Select a PDF/TXT file on the left sidebar. The system now validates files strictly.
...
This project includes a test suite to ensure the pipeline handles data correctly and rejects invalid inputs safely.
...
To ensure system stability, the ingestion engine enforces a strict schema. Any `Document` object processed by the system **MUST** contain the following metadata keys:

| Key | Type | Description |
| --- | --- | --- |
| `source` | `str` | Origin filename (e.g., `lecture_notes.pdf`). |
| `type` | `str` | File format (e.g., `pdf`, `txt`). |
| `id` | `str` | (Optional) Unique UUID for the chunk. |

**Failure Mode:**
If a document is missing `source` or `type`, the `IngestionEngine` will raise a `ValueError` to prevent "silent failures" downstream.
...
For detailed engineering notes on limitations, expected failure modes, and architecture decisions, please refer to **[HANDOVER.md](https://www.google.com/search?q=./HANDOVER.md)**.
